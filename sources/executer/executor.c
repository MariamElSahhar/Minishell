...
/*   Updated: 2023/06/01 20:04:25 by szerisen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	executor(t_utils *utils)
{
	...
	while (utils->cmds)
	{
		...
		if (utils->cmds->next)
			pipe(end);
		send_heredoc(utils, utils->cmds);
		ft_fork(utils, end, fd_in, utils->cmds);
		close(end[1]);
		if (utils->cmds->prev)
			close(fd_in);
		fd_in = check_fd_heredoc(utils, end, utils->cmds);
		if (utils->cmds->next)
	...
	utils->cmds = ft_cmdsfirst(utils->cmds);
	return (0);
}
/*
Added Additional
