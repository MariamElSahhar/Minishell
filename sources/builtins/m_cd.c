...
int	m_cd(t_utils *utils, t_cmds *cmds)
{
	...
	// checking if the first argument (cmds->args[1]) starts with a hyphen ("-"). This means the user wants to go to previous directory.
	...
	else
	{
		...
		if (ret != 0)
		{
			...
			// this line is used to print the value of cmds->args[1] to the standard error stream.
